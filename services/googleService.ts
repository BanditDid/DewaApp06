...
// Helper: ดึงค่า Env อย่างปลอดภัย (รองรับทั้ง Next.js และ Vite)
const getEnv = (key: string): string => {
    try {
        // กรณี Next.js / Create React App (process.env)
        if (typeof process !== 'undefined' && process.env && process.env[key]) {
            return process.env[key] as string;
        }
        // กรณี Vite (import.meta.env)
        // @ts-ignore
...
// State: เก็บค่า Config ปัจจุบัน
// เราจะอ่านค่าจาก NEXT_PUBLIC_... เป็นหลักสำหรับโปรเจกต์นี้
let currentConfig: GoogleConfig = {
    clientId: getEnv('NEXT_PUBLIC_GOOGLE_CLIENT_ID'),
    apiKey: getEnv('NEXT_PUBLIC_GOOGLE_API_KEY')
};
...
export const GoogleService = {
  ...
  initClient: async (): Promise<void> => {
    ...
    // ตรวจสอบ Config ผ่าน Helper ใหม่
    ...
    const { apiKey, clientId } = currentConfig;
